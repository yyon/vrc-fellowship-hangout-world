...
namespace CentauriCore.Blackjack
{
    ...
    {
        [Header("Dealer Controller")]
        [Space(20)]
        public bool IsBust;
        [Space]
        [UdonSynced] public int TotalDeckValue;
        private int localDeckValue;
        public int[] DeckValue = new int[11];
        private Animator[] DeckAnimators = new Animator[11];
        [Space] 
        public int NextDeckPosition = 0;
        private int LastDeckPosition = -1;
        ...
        [Space] 
        //[UdonSynced] public int NextCardToSpawn = -1;
        //private int LastCardToSpawn;
        ...
        private bool hasFlippedCard = false;
        ...
        public void DealerPlay()
        {
            if (!hasFlippedCard)
            {
                SendCustomNetworkEvent(NetworkEventTarget.All, nameof(GlobalFlip));

                hasFlippedCard = true;
                
                GameController.AudioController.PlayFlipCard();
                
                SendCustomEventDelayedSeconds(nameof(DealerPlay), 1f);
                return;
            }
            
            if (!IsBust && localDeckValue < 17)
            {
                SummonCard();
                
                GameController.AudioController.PlayDrawCard();
                
                SendCustomEventDelayedSeconds(nameof(DealerPlay), 0.5f);
                return;
            }
            ...
            if (IsBust)
            {
                ...
                DealerUI.SendCustomNetworkEvent(NetworkEventTarget.All, "Bust");
            }

            hasFlippedCard = false;
            
            GameController.FinishMatch();
        }

        public void GlobalFlip()
        {
            flippedCard.GetComponent<Animator>().SetTrigger("Flip");
        }
        ...
        {
            Debug.Log("Starting match with Dealer");
        ...
        public void SummonCard()
        {
            ...
            {
                SendCustomEvent("NetworkSpawnCard" + cardIndex + "DeckPos" + NextDeckPosition);
                NextDeckPosition++;
            }
            else
            {
                NextCardToSpawn = cardIndex;
        ...
        {
            if (Networking.IsOwner(gameObject) || !GameController.localGameActive) return;
        ...
        /*
            if (NextCardToSpawn > LastCardToSpawn && NextDeckPosition > LastDeckPosition)
            {
                RemoteAddCard();
            }*/
        ...
        public override void OnPostSerialization(SerializationResult result)
        {
            if (NextCardToSpawn > LastCardToSpawn && NextDeckPosition > LastDeckPosition)
            {
                SendCustomEvent("NetworkSpawnCard" + NextCardToSpawn + "DeckPos" + NextDeckPosition);
                LastCardToSpawn = NextCardToSpawn;
                LastDeckPosition = NextDeckPosition;
